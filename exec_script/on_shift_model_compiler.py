...
bucket = np.zeros([int(real_time_speed.tail(1)["secs_elapsed"]
                       * switch_ratio), len(feature_columns)], dtype=float)
...
w1 = WindowGenerator(input_width=24, label_width=1, shift=24,
                     train_df=train_df, val_df=val_df, test_df=test_df, label_columns=["qps"])
...
print("Single shift, single step training time cost: %.2f s" %
...
print("Single shift, three step training time cost: %.2f s" %
      (float(end-start)/1000000000))
...
print("Single shift, three step mae loss: %.4f" %
      performance['Multi step dense'][1])
...
print("Single shift, Conv training time cost: %.2f s" %
      (float(end-start)/1000000000))
...
wide_window = WindowGenerator(input_width=60, label_width=60, shift=1,
                              train_df=train_df, val_df=val_df, test_df=test_df, label_columns=['qps'])
...
print("Single shift, LSTM training time cost: %.2f s" %
      (float(end-start)/1000000000))
